...
use iced::window::{Id, RedrawRequest};
use iced::{mouse, theme, Color, Point, Program, Rectangle, Size, Vector};
    ...
    P::Theme: theme::Base,
{
    ...
    pub fn insert(
        ...
        tracing::info!("WindowManager::insert: id={:?}", id);

        let state = state::State::new(program, id, &window, system_theme);
    ...
    P::Theme: theme::Base,
{
    ...
    ime_state: Option<(iced::Rectangle, iced::advanced::input_method::Purpose)>,
}
    ...
    P::Theme: theme::Base,
{
    ...
    fn enable_ime(&mut self, cursor: Rectangle, purpose: iced::advanced::input_method::Purpose) {
    ...
    Renderer: iced::advanced::text::Renderer,
{
    ...
    spans: Vec<iced::advanced::text::Span<'static, (), Renderer::Font>>,
}
    ...
    Renderer: iced::advanced::text::Renderer,
{
    ...
    fn update(
        &mut self,
        cursor: Rectangle,
        preedit: &iced::advanced::input_method::Preedit,
        background: Color,
        renderer: &Renderer,
    ) {
        ...
        let spans = match &preedit.selection {
            Some(selection) => {
                vec![
                    iced::advanced::text::Span::new(&preedit.content[..selection.start]),
                    iced::advanced::text::Span::new(if selection.start == selection.end {
                        "\u{200A}"
                    } else {
                        &preedit.content[selection.start..selection.end]
                    })
                    .color(background),
                    iced::advanced::text::Span::new(&preedit.content[selection.end..]),
                ]
            }
            _ => vec![iced::advanced::text::Span::new(&preedit.content)],
        };

        if spans != self.spans.as_slice() {
            use iced::advanced::text::Paragraph as _;

            self.content = Renderer::Paragraph::with_spans(iced::advanced::text::Text {
                content: &spans,
            ...
            self.spans.clear();
            self.spans
                .extend(spans.into_iter().map(iced::advanced::text::Span::to_static));
        }
    }
