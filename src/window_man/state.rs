use iced::{mouse, theme, window};
use iced::{Color, Size};
use iced_winit::conversion;
use iced_winit::graphics::Viewport;
use iced_winit::program::{self, Program};
    ...
    P::Theme: theme::Base,
{
    ...
    pub fn new(
        ...
        let viewport = {
            ...
            #[cfg(target_os = "ios")]
            let viewport_scale_factor = window.scale_factor() as f32;

            #[cfg(not(target_os = "ios"))]
            let viewport_scale_factor = window.scale_factor() as f32 * scale_factor;
    ...
    pub fn update(&mut self, program: &program::Instance<P>, window: &Window, event: &WindowEvent) {
        match event {
            WindowEvent::Resized(new_size) => {
                ...
                #[cfg(not(target_os = "ios"))]
                let viewport_scale_factor = window.scale_factor() as f32 * self.scale_factor;

                self.viewport = Viewport::with_physical_size(size, viewport_scale_factor);
                self.surface_version += 1;

                tracing::debug!(
                    "Window resized: physical_size={:?}, scale_factor={}",
                    size,
                    viewport_scale_factor
                );
            }
            WindowEvent::ScaleFactorChanged {
                ...
                #[cfg(target_os = "ios")]
                let viewport_scale_factor = *new_scale_factor as f32;

                #[cfg(not(target_os = "ios"))]
                let viewport_scale_factor = *new_scale_factor as f32 * self.scale_factor;

                self.viewport = Viewport::with_physical_size(size, viewport_scale_factor);
                self.surface_version += 1;

                tracing::debug!(
                    "Scale factor changed: new_scale={:?}, viewport_scale_factor={}",
                    new_scale_factor,
                    viewport_scale_factor
                );
            }
    ...
    pub fn synchronize(
        ...
        if self.scale_factor != new_scale_factor {
            ...
            #[cfg(not(target_os = "ios"))]
            let viewport_scale_factor = window.scale_factor() as f32 * new_scale_factor;

            self.viewport =
                Viewport::with_physical_size(self.viewport.physical_size(), viewport_scale_factor);
